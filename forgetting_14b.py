...
EVAL_N       = 19
STEPS        = 200
BATCH        = 2        # smaller batch per GPU for 14B
LR           = 2e-5
SEED         = 42
DTYPE        = torch.bfloat16
INPUT_DEV    = "cuda:0"   # input tensors; model layers spread across all GPUs
N_GPUS       = torch.cuda.device_count()
OUT          = "/root/autodl-tmp/results_14b.json"
...
print(f"Model: {MODEL_ID}  device_map=balanced")
...
# ── Main ──────────────────────────────────────────────────────────────────────
tok = AutoTokenizer.from_pretrained("/root/autodl-tmp/Qwen2.5-14B", local_files_only=True)
...
t0 = time.time()
print(f"\n{'='*60}")
print(f"Running 6 pairs  [{MODEL_ID}]  {N_GPUS} GPUs")
print(f"{'='*60}")
...
for da, db in permutations(dom.keys(), 2):
    ...
    model = AutoModelForCausalLM.from_pretrained(
        "/root/autodl-tmp/Qwen2.5-14B", dtype=DTYPE, device_map="balanced", local_files_only=True)
...
try:
    ...
    print(f"vs 3B baseline: {ps:.1f}pp -> {sp:.1f}pp  ranking_stable={pr==cr}")
except FileNotFoundError:
    pass

results["_summary"] = {"model": MODEL_ID, "n_gpus": N_GPUS,
