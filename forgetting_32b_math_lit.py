...
INPUT_DEV    = "cuda:0"
OUT          = "/root/autodl-tmp/results_32b_math_lit.json"
    ...
    while step < STEPS:
        for b in blist:
            ...
            try:
                ...
                print(f"  OOM at step {step}! [{vram()}]", flush=True)
                opt.zero_grad()
                for i in range(N_GPUS):
...
print(f"  ppl(math) before lit: {ppl_before:.3f}", flush=True)

finetune(model, tb, "B=literature")
ppl_after = ppl(model, ea)
print(f"  ppl(math) after lit:  {ppl_after:.3f}", flush=True)
