...
# ── Bake in all env vars before any HF import ─────────────────────────────────
...
EVAL_N       = 19
STEPS        = 200
BATCH        = 16           # 4 GPUs × 4 — fills memory, maximises throughput
LR           = 2e-5
SEED         = 42
DTYPE        = torch.bfloat16
N_GPUS       = torch.cuda.device_count()
INPUT_DEV    = "cuda:0"     # input tensors; layers spread across all GPUs
OUT          = "/root/autodl-tmp/results_3b.json"
PREV         = "/root/results_controlled.json"
...
print(f"Model: {MODEL_ID}  device_map=balanced  batch={BATCH}")
...
# ── Main ──────────────────────────────────────────────────────────────────────
tok = AutoTokenizer.from_pretrained("/root/autodl-tmp/Qwen2.5-3B", local_files_only=True)
...
for name, texts in raw.items():
    ...
    print(f"  {name}: {len(dom[name]['train'])} train, {len(dom[name]['eval'])} eval chunks")

results = {}
t0 = time.time()
print(f"\n{'='*60}")
print(f"Running 6 pairs  [{MODEL_ID}]")
print(f"{'='*60}")
...
for da, db in permutations(dom.keys(), 2):
    ...
    model = AutoModelForCausalLM.from_pretrained(
        "/root/autodl-tmp/Qwen2.5-3B", dtype=DTYPE,
...
try:
    ...
    print(f"vs 0.5B baseline: {ps:.1f}pp -> {sp:.1f}pp  ranking_stable={pr==cr}")
except FileNotFoundError:
    pass

results["_summary"] = {"model": MODEL_ID, "spread_pct": round(sp, 4),
                        "min_pair": min_p, "max_pair": max_p,
